function Ship(){  //constructor func.  with its obj 
  ...
  //ASSIGNING PROPERTIES TO OBJECT SHIP
  ...
  this.update= function(){ // it will be updating ship movement every time by velocity

    if(this.isBoosting){ // if its boosting it will call boost()
      this.boost();
    }
  ...
  this.boost=function(){   
   ...
   var force= p5.Vector.fromAngle(this.heading); // it will help the ship to move forward by adding force
    force.mult(0.2);
  ...
  this.render= function(){  //render func. will show outputs in display by translating  x y points graph
  ...
  this.edges=function(){  // to make it reappare when its off the screen
    // width
